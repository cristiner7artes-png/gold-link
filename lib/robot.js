...
async function refill(col, cfg) {
  ...
  for (const categoria of cfg.categoriasPermitidas) {
    ...
    // Busca alguns candidatos a mais do que falta (para sobrar após os filtros),
    // respeitando o "máx. por categoria" configurado no admin.
    const fetchLimit = Math.min(50, Math.max(12, faltam + 6));

    let ofertas;
    try {
      ofertas = await searchCategoryOffers(categoria, { limit: fetchLimit, onlyDeals: true });
    } catch (e) {
