...
// Categorias que realmente existem na vitrine — o robô só deve buscar destas.
...
export async function searchCategoryOffers(categoryLabel, { limit = 12, onlyDeals = true } = {}) {
  ...
  // The admin-configured "máx. por categoria" drives how many best-sellers we
  // resolve. We cap at 50 to protect against timeouts (each product costs 3 API
  // calls: product + items + reviews). The highlights list is naturally bounded
  // by whatever the category actually returns.
...
const DEFAULT_AFFILIATE = 'matt_word=costamarcelo20231025021523&matt_tool=36468671';
